...
int main(void)
{
	...
	while(1)
	{
		joyVal = adc_read(0); //read joystick output - 10bit value | 0-1023
		joyScale = (joyVal>>2); //bitshift down 2 - 8bit value | 0-255
		if(joyScale > 253){ //Limits highest value to 253 | 0-253
			joyScale = 253;
		}
		...
		sensorScale = dataByte1; //Scaled sensor value received from Robot | Not distance, just voltage
		lcd_home();	//position 0
		sprintf(lcd_string, "%d", sensorScale);	//format for lcd
		lcd_puts(lcd_string);	//print to lcd
	}
	return(1);
} //end main
...
ISR(USART2_RX_vect)  // ISR executed whenever a new byte is available in the serial buffer
{
	...
	switch(serial_fsm_state) //switch by the current state
	{
		...
		if(serial_byte_in == 0xFE) //stop byte
		{
			...
			dataByte1 = recvByte1; //Sensor value from 0-253
			dataByte2 = recvByte2;
			dataByte3 = recvByte3;
			dataByte4 = recvByte4;
